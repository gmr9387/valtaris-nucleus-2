...
import { a as stringType, n as enumType, r as objectType, s as ZodIssueCode } from "../_libs/zod.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/schemas-BohSmhqW.js
...
var rotationReasonSchema = enumType([
	"scheduled",
	"manual",
	"compromised",
	"policy",
...
var nullableUuid = stringType().uuid().nullable().optional().transform((value) => value ?? null);
var secretValueSchema = stringType().min(8, "Secret must be at least 8 characters").max(8192, "Secret is too large").refine((value) => value.trim().length >= 8, { message: "Secret cannot be blank or mostly whitespace" });
var createCredentialSchema = objectType({
	organization_id: stringType().uuid(),
	provider_id: stringType().uuid(),
...
var rotateCredentialSchema = objectType({
	credential_id: stringType().uuid(),
	new_secret: secretValueSchema,
	reason: rotationReasonSchema.default("manual")
});
var deactivateCredentialSchema = objectType({ credential_id: stringType().uuid() });
objectType({
	organization_id: stringType().uuid(),
	connector_id: stringType().uuid(),
...
//#endregion
export { rotateCredentialSchema as a, requireSupabaseAuth as i, createCredentialSchema as n, deactivateCredentialSchema as r, buildRedactedPreview as t };
