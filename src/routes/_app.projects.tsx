  ...
  const create = useMutation({
    mutationFn: async () => {
      ...
      const { data, error } = await supabase.from("projects").insert({
        organization_id: currentOrgId!, name: parsed.name, slug: parsed.slug,
        description: parsed.description ?? null, created_by: user!.id,
      }).select().single();
      if (error) throw error;
      await logAudit({ organization_id: currentOrgId, module: "tenancy", entity_type: "project", entity_id: data.id, action: "create", after: data });
      return data;
    },
    onSuccess: () => {
  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          <button onClick={() => setOpen((v) => !v)} disabled={!currentOrgId}
      ...
      <PageBody>
        {open && (
          <form onSubmit={(e) => { e.preventDefault(); create.mutate(); }} className="mb-6 rounded-lg border border-border bg-surface-1 p-4">
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              <Field label="Name">
                <input className="input" value={name} onChange={(e) => { setName(e.target.value); setSlug(e.target.value.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "").slice(0, 60)); }} required />
              </Field>
              <Field label="Slug">
              ...
              <div className="md:col-span-2">
                <Field label="Description">
                  <textarea className="input" style={{ height: 72, paddingTop: 8 }} value={desc} onChange={(e) => setDesc(e.target.value)} maxLength={500} />
                </Field>
              </div>
            </div>
        ...
        {list.data?.length ? (
          <div className="overflow-hidden rounded-lg border border-border">
            <table className="w-full text-sm">
              <thead className="bg-surface-1 text-mono-xs text-muted-foreground">
                <tr><Th>Name</Th><Th>Slug</Th><Th>Status</Th><Th>Created</Th></tr>
              </thead>
              <tbody className="divide-y divide-border bg-surface-1/40">
                {list.data.map((p) => (
                  <tr key={p.id} className="hover:bg-surface-2/60">
                    <Td><div className="font-medium">{p.name}</div>{p.description && <div className="text-xs text-muted-foreground">{p.description}</div>}</Td>
                    <Td><span className="font-mono text-xs text-muted-foreground">{p.slug}</span></Td>
                    <Td><span className="inline-flex items-center gap-1.5"><StatusDot status={p.status} /><span className="capitalize">{p.status}</span></span></Td>
                    <Td><span className="text-muted-foreground">{new Date(p.created_at).toLocaleDateString()}</span></Td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <EmptyState title="No projects yet" description="Create a project to organize environments and resources." />
        )}
        <FieldStyles />
      </PageBody>
    </>
